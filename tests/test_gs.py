...
class GoogleSpreadsheetLoadDataToDataframe(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_df = pd.read_csv('tests/csv/test_load_to_gs.csv')
    ...
    def test_load_gs_to_dataframe(self):
        result_by_sheet_name = gs.load_gs_to_dataframe(self.spreadsheet.id, sheet_name='new_worksheet')
        assert_frame_equal(
            result_by_sheet_name,
            self.test_df
        )

        result_by_sheet_index = gs.load_gs_to_dataframe(self.spreadsheet.id, sheet_index=1)
        assert_frame_equal(
